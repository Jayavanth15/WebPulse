...
def create_userorg(userorg: UserOrgCreate, db: Session = Depends(get_db), cred = Depends(decode_token)):
    if cred[0] in roles:
...
def update_userorg(id: int, userorg: UserOrgCreate, db: Session = Depends(get_db), cred = Depends(decode_token)):
    if cred[0] in roles:
...
def delete_userorg(id: int, db: Session = Depends(get_db), cred = Depends(decode_token)):
    if cred[0] in roles:
