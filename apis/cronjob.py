...
async def create_cronjob(id: int, db: Session = Depends(get_db), cred=Depends(decode_token)):
...
async def delete_cronjob(id: int, db: Session = Depends(get_db), cred=Depends(decode_token)):
