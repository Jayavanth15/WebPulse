...
def create_site(site: WebCreate, db: Session = Depends(get_db), cred = Depends(decode_token)):
    if cred[0] in roles:
...
def update_URL(site_id: int, site: WebCreate, db: Session = Depends(get_db), cred = Depends(decode_token)):
    if cred[0] in roles:
...
def delete_URL(site_id: int, db: Session = Depends(get_db), cred = Depends(decode_token)):
    if cred[0] in roles:
