...
def create_org(org : OrgCreate, db : Session = Depends(get_db), cred = Depends(decode_token)): 
    if cred[0] in roles:
...
def update_org(org_id : int, new_org : OrgCreate, db : Session = Depends(get_db), cred = Depends(decode_token)):
    if cred[0] in roles:
...
def delete_org(org_id : int, db : Session = Depends(get_db), cred = Depends(decode_token)):
    if cred[0] in roles:
        exist_org = db.query(Organisation).filter(Organisation.org_id == org_id).first()
        if not exist_org:
            raise HTTPException(status_code=400, detail="Organisation not found")
