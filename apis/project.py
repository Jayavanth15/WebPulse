...
def create_project(project : ProjectCreate, db : Session=Depends(get_db), cred = Depends(decode_token)):
    if cred[0] in roles:
...
def update_project(project_id : int, project : ProjectCreate, db : Session=Depends(get_db), cred = Depends(decode_token)):
    if cred[0] in roles:
...
def delete_project(project_id : int, db : Session=Depends(get_db), cred = Depends(decode_token)):
    if cred[0] in roles:
        exist_project = db.query(Project).filter(Project.project_id == project_id).first()
        if not exist_project:
            raise HTTPException(status_code=404, detail="Project does not exists")
